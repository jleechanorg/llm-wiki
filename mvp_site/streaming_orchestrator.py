...
def _enrich_streaming_structured_fields(  # noqa: PLR0912
    ...
    *,
    ...
    if "world_events" not in structured_fields:  # noqa: SIM102
        if isinstance(state_updates, dict):
            ...
            used_nested_custom_world_events = False
            if (
                "world_events" not in state_updates
                or not isinstance(world_events, dict)
                or not world_events
            ):
                custom_state = state_updates.get("custom_campaign_state", {})
                if isinstance(custom_state, dict):
                    nested_we = custom_state.get("world_events")
                    if isinstance(nested_we, dict):
                        world_events = nested_we
                        used_nested_custom_world_events = True
            if isinstance(world_events, dict) and world_events:
                structured_fields["world_events"] = world_events
                if used_nested_custom_world_events:
                    cs = state_updates.get("custom_campaign_state")
                    if isinstance(cs, dict):
                        cs.pop("world_events", None)
    ...
    # BD-724 parity with world_logic.process_action_unified: server warnings from
    # debug_info._server_system_warnings must be copied to structured_fields so
    # add_story_entry persists them for reload (dice integrity, validation, etc.).
    # SECURITY: Only _server_system_warnings is trusted (same as non-streaming path).
    _merge_server_system_warnings_for_streaming_persistence(structured_fields)


def _merge_server_system_warnings_for_streaming_persistence(
    structured_fields: dict[str, Any],
) -> None:
    """Copy server-only warnings into persisted system_warnings for Firestore story entries."""
    debug_info = structured_fields.get("debug_info")
    if not isinstance(debug_info, dict):
        return
    server_warnings = debug_info.get("_server_system_warnings", [])
    if not isinstance(server_warnings, list) or not server_warnings:
        return
    deduped = list(dict.fromkeys(server_warnings))
    structured_fields["system_warnings"] = deduped
...
def stream_story_with_game_state(  # noqa: PLR0912, PLR0915
    ...
    try:
        ...
        # Mirror the non-streaming path so modal level-up choices activate the
        # level-up lock before agent selection runs in llm_service.
        world_logic._maybe_trigger_level_up_modal(
            user_input, story_context, current_game_state
        )
        ...
        for event in continue_story_streaming(
            ...
            pre_response_state_dict = (
                current_game_state.to_dict()
                if isinstance(current_game_state, GameState)
                else {}
            )

            # Initialize canonical stream vars before the try so they are always
            # defined even if an exception aborts the state-update block early.
            canonical_stream_rewards_box: dict[str, Any] | None = None
            canonical_stream_planning_block: dict[str, Any] | str | None = None
            canonical_stream_suppressed: bool = False

            try:
                state_dirty = False
                updated_state_dict = copy.deepcopy(pre_response_state_dict)
                # CRITICAL: Enforce the modal lock UNCONDITIONALLY, mirroring
                # the non-streaming path in world_logic.process_action_unified.
                # An empty ``state_updates`` dict is falsy in Python, so
                # guarding the call with ``if state_updates:`` would skip
                # server-authored transitions (e.g. the finish_level_up exit
                # choice) whenever the LLM omits state_updates, leaving the
                # modal permanently stuck in the streaming path.
                state_updates = state_updates or {}
                state_updates = world_logic._enforce_character_creation_modal_lock(
                    pre_response_state_dict,
                    state_updates,
                    user_input,
                )
                event.payload["state_updates"] = state_updates
                if state_updates:
                ...
                # Canonicalize the rewards_box/planning_block pair BEFORE
                # persisting the planning_block to ``updated_state_dict``.
                # Otherwise Firestore keeps the raw LLM planning_block that
                # the polling path reads, even when the canonical resolver
                # (applied to ``gemini_structured`` further down) suppresses
                # or rewrites the pair.
                raw_structured_rewards_box = None
                raw_structured_planning_block = None
                if isinstance(structured_fields, dict):
                    raw_structured_rewards_box = structured_fields.get("rewards_box")
                    raw_structured_planning_block = structured_fields.get(
                        "planning_block"
                    )
                ...
                if resolved_rb is not None:
                    ...
                    if world_logic._has_level_up_ui_signal(
                        ...
                        if world_logic._planning_has_level_up_choices(
                            raw_structured_planning_block
                        ):
                            canonical_stream_planning_block = None
                        else:
                            canonical_stream_planning_block = resolved_pb
                    else:
                        canonical_stream_planning_block = resolved_pb

                # Apply modal-finish injection to the canonical planning block
                # BEFORE writing to game state so the persisted state and the
                # streamed done payload both reflect the same post-injection block.
                if canonical_stream_planning_block is not None:
                    _modal_finish_injected = (
                        world_logic._inject_modal_finish_choice_if_needed(
                            canonical_stream_planning_block,
                            pre_response_state_dict,
                        )
                    )
                    if _modal_finish_injected is not None:
                        canonical_stream_planning_block = _modal_finish_injected

                if canonical_stream_planning_block is not None:
                    updated_state_dict["planning_block"] = (
                        canonical_stream_planning_block
                    )
                    state_dirty = True
                elif canonical_stream_suppressed:
                    # Clear any stale raw planning_block that the polling path
                    # would otherwise resurrect after suppression.
                    if updated_state_dict.get("planning_block") is not None:
                        updated_state_dict["planning_block"] = None
                        state_dirty = True
                ...
                try:
                    ...
                    if not validation_failed and validated_narrative:
                        ...
                        if modal_finish_injected_block is not None:
                            gemini_structured["planning_block"] = (
                                modal_finish_injected_block
                            )
                        # Reuse the canonical pair already resolved before
                        # persistence so Firestore game state and the done
                        # payload stay aligned.
                        # When a modal-finish block was injected, preserve it
                        # and only fall back to canonical_stream_planning_block
                        # when there was no injection.
                        if canonical_stream_suppressed:
                            gemini_structured.pop("rewards_box", None)
                            if modal_finish_injected_block is not None:
                                # Keep the injected finish choice; don't suppress it.
                                pass
                            elif canonical_stream_planning_block is None:
                                gemini_structured.pop("planning_block", None)
                            else:
                                gemini_structured["planning_block"] = (
                                    canonical_stream_planning_block
                                )
                        elif canonical_stream_rewards_box is not None:
                            gemini_structured["rewards_box"] = (
                                canonical_stream_rewards_box
                            )
                            if (
                                modal_finish_injected_block is None
                                and canonical_stream_planning_block is not None
                            ):
                                gemini_structured["planning_block"] = (
                                    canonical_stream_planning_block
                                )

                        event.payload["structured_response"] = gemini_structured
                        firestore_service.add_story_entry(
                            user_id,
                            campaign_id,
                            "gemini",
