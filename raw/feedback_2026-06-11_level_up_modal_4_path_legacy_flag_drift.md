---
name: canonical-field-deprecation-requires-auditing-all-legacy-readers
description: "When a canonical field deprecates a legacy field, all backend readers of the legacy field must be migrated in the same pass, not just the most obvious one (modal lock routing)"
metadata: 
  node_type: memory
  type: feedback
