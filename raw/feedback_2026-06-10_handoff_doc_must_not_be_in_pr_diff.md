---
name: handoff-doc-must-not-be-in-pr-diff
description: "Never push a roadmap handoff doc to a code PR's branch — the Skeptic agent (Gate 8) will reject the PR for 'Scope boundary gap' because handoff docs reference cross-PR files. Put handoff docs in a separate branch or in ~/roadmap/ outside the repo."
metadata: 
  node_type: memory
  type: feedback
