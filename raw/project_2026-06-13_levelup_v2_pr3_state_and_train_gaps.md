---
name: project_2026-06-13_levelup_v2_pr3_state_and_train_gaps
description: "Level-up v2 PR-3 (#7530 rewards_engine shim) landed + two systemic train-level gaps that block organic routing"
metadata: 
  node_type: memory
  type: project
  originSessionId: 62b1445e-8bb3-4ba6-9dba-f2a370e4c262
---
...
**Landed this session (rewards_engine.py lane only):**
...
**TWO SYSTEMIC TRAIN-LEVEL GAPS (~55 cross-lane tests red, NOT PR-3's job):**
...
**Other gate notes:** no `holdout` eval harness exists in the repo by that name. `/code_standards` = 4 ZFC review lanes (.claude/skills/code-standards). `/er` (.claude/skills/evidence-review.md) needs a full evidence bundle (GIF+MP4+checksums+gist) to PASS — heavyweight, not run.

**Gate 0 fix (2026-06-13 follow-up session):** The failing gate was Design Doc Gate 0. PR-3's non-test py delta is ~533 lines (>50 threshold), and the `## Tenets` section had NO `.md`/`rev-` artifact link (the governing-doc URL lived in `## Background`, which the awk extractor ignores — it only reads the `## Tenets`/`## Design Decision` section, stopping at the next `##`). NOTE: the body's governing doc `docs/plans/2026-06-13-level-up-v2-immediate-commit.md` does NOT exist on the branch or origin/main (404). Fixed by adding a bare path `roadmap/zfc-level-up-model-computes-2026-04-19.md` (real file, exists on branch+main, is the ZFC anchor the gate workflow header itself cites) into the Tenets section via `gh pr edit --body-file` (no SHA change; `edited` event re-runs the gate). Simulated the gate's awk+grep locally → artifact matched. 323 rewards_engine tests still green, 14/14 v2 shim green. See [[feedback_2026-06-13_design_doc_gate0_artifact_inside_tenets]].
