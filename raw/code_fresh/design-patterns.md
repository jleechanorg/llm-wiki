...
class NotificationFactory {
  ...
  // Note: This simple Factory violates Open/Closed Principle (modifying switch to add types).
  // This is an acceptable trade-off for centralizing creation logic.
}
```
...
class UserBuilder {
  ...
  build(): User {
    if (!this.user.name || !this.user.email) throw new Error('Missing required fields');
