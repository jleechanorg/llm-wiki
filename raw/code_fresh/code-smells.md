...
| **Speculative Generality** | "Just in case" code (often violates ISP) | Delete (YAGNI) |
...
// Note: Specific, domain-aligned Service names are acceptable. Avoid generic names like "Manager" or "Handler".
...
class Order {
  process() {
    ...
    // Reaching into inventory's internals - knowing too much about implementation
    for (const item of this.items) {
      if (!inventory.hasStock(item.sku)) { // hypothetical method
         // ... 
      }
      // Direct field access is the smell
