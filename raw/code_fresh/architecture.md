...
**Why:** Changes to "users" feature stay in `users/`. High cohesion within features. Preferred for most modern applications, though Layer-first can work for very small or purely technical libraries.
...
- User can view ONE product (hardcoded - acceptable only in this phase)
- User can add it to cart
- User can "checkout" (just logs)

From there, flesh out each feature fully and **systematically replace hardcoded/stubbed values** with real data sources and configuration. Hardcoding is acceptable at the walking-skeleton stage only as short-lived scaffolding that you remove incrementally.
