...
### 8. Avoid Large State (Aim for < 5 Instance Variables)

Forces small, focused classes. If you need more, you likely need a new object.

> Note: The \"no more than two instance variables\" rule is a design heuristic aimed at
> behavior-rich domain objects. When a class starts to accumulate many different fields,
> that is a signal to extract smaller concepts (like `OrderDetails`) and compose them.
> Small, cohesive value objects that simply group closely related data (such as an
> `Address` with `street`, `city`, `state`, `zipCode`) can legitimately have more than
> two fields without violating the spirit of this rule.
