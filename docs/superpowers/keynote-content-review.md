...
**Live deck:** https://docs.google.com/presentation/d/1Zb6sc0HUKOIR-As3fDl68iYEs6FFFrzdi_YVVo89Xa0/edit — **PUSHED, final 18 slides** as of 2026-08-23 ~17:24 PT (14 main-flow + 4 appendix). Assembly: appended all 18 new slides to the end first (10 freshly rendered from `keynote/deck/*.html` via 1335×896 browser screenshot, 8 unchanged originals reused directly from this session's cached renders), verified count (35 = 17 original + 18 new), then deleted the original 17 by their known object IDs, verified final count (18). Exported a fresh PDF and spot-checked slides 1 (new intro-bio), 5 (unchanged pipeline), 9 (fixed pyramid), 14 (new find-me, footer correctly "14/14"), and 18 (unchanged appendix) — all pixel-correct. **Known trade-off:** the 10 freshly-rendered slides are at 1335×896 (browser screenshot tool's fixed capture size, `resize_window` to 1920×1080 didn't change it) vs. the original's 2000×1125 PDF-export quality — a ~1.44x upscale onto the 1440×810pt slide canvas. Content is complete and correctly laid out, just slightly softer than the vector-quality original renders; flag if you want a higher-res re-render pass before presenting.
