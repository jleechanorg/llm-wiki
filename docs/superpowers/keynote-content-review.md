...
**Resolution fix (round 1):** the first push used the claude-in-chrome extension's screenshot tool, fixed at 1335×896 regardless of `resize_window` — a visible upscale on the slide canvas. Fixed by rendering the same 10 pages via local headless Chrome (`--window-size=1920,1080 --force-device-scale-factor=2 --screenshot=...`) instead, producing crisp 3840×2160 captures.

**Crop bug (round 2), found by a 6-way parallel subagent visual-QA sweep (`/visual-verification`) + my own direct re-inspection:** swapping those hi-res images in via `gog slides replace-slide` silently mis-sized/mis-positioned every one of the 10 slides — left/right edges and the entire footer row were cropped off-canvas on all 10 (confirmed by reading the actual exported PDF pages myself, not just trusting subagent reports — one subagent's "node 4 label missing" and "pill tags missing" findings turned out to be the same crop bug, not independent defects). Root cause: `replace-slide` doesn't take `--width`/`--unit` sizing flags the way `insert-image` does, so it re-sized based on the new image's native 3840×2160 pixels instead of the slide's 1440×810pt frame. Fix: deleted all 10 broken slides, re-added them via the already-proven `new-slide` + `insert-image --width=1440 --unit=PT` (same command used successfully in the original bulk push, this time with the hi-res source images), then used `gog slides move-slide --to-index=N` to restore the correct running order. Re-exported and personally read all 18 slides end-to-end — full margins, footers, page numbers, and pill tags present and sharp on every one.
