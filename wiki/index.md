...
- [PR 7467 review — generic prompt fixes + live PR head + readiness gates — 2026-06-12](sources/feedback-2026-06-12-pr-7467-prompt-and-readiness.md) — 3 intertwined lessons: (1) prompt content fixes must be generic rules, not class-specific lists (rework `7610402bc3` over-fit); (2) `git fetch && git rev-parse origin/<branch>` before claiming live head (local ≠ live on multi-writer PRs); (3) PR readiness is 6 gates, not "looks good" — body evidence, CI green, threads resolved, CodeRabbit APPROVED, real-LLM PASS at live head, Skeptic for live head
