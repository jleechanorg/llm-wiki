...
- Verified codebases (see [FormalVerification](FormalVerification.md)) provide a reliable ground truth that makes self-debugging more reliable: the model can use the specification as a target for its fixes
...
- [SelfCritique](SelfCritique.md) — self-debugging subsumes self-critique: critique identifies problems, debugging fixes them
- [AdversarialTesting](AdversarialTesting.md) — adversarial testing finds the bugs that self-debugging then fixes
- [VerificationLoop](VerificationLoop.md) — self-debugging is the "fix" stage in a verification loop, preceded by verification and adversarial attack
- [SkepticGate](SkepticGate.md) — skeptic gates can trigger self-debugging passes when verification fails
- [AgenticCoding](AgenticCoding.md) — self-debugging is the natural extension of agentic coding for post-generation refinement
- [HarnessEngineering](HarnessEngineering.md) — a harness implementing self-debugging requires robust error capture and re-execution infrastructure

## Relationships to Other Concepts

Self-debugging is the most sophisticated member of the self-repair family. A [HarnessEngineering](HarnessEngineering.md) effort implementing self-debugging would typically layer it on top of [ExtendedThinking](ExtendedThinking.md) (for root cause analysis) and [ReasoningBudget](ReasoningBudget.md) (to allow sufficient compute for complex debugging sessions).
