...
- [feedback-2026-06-28-runner-session-conflict](../sources/feedback-2026-06-28-runner-session-conflict.md) — single-runner variant of the GH-side `busy=true` corruption; container is Up + Listening while gh API shows offline; Runner.Listener "Error: Conflict. Retrying until reconnected." loop. Heal = DELETE stale runner ID + `docker restart` + verify `status:"online"`. See [RunnerSessionConflict](RunnerSessionConflict.md) for the focused pattern + verification hierarchy.
