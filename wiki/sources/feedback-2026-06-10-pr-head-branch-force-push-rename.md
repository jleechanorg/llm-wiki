---
title: "Feedback 2026 06 10 Pr Head Branch Force Push Rename"
type: source
tags: [memory, ao-worker, codex-worker, 2026-06]
date: 2026-06-10
source_file: .claude/projects/-Users-jleechan-projects-worldarchitect-ai/memory/feedback_2026-06-10_pr_head_branch_force_push_rename.md
---

## Summary

When you need to change a PR's underlying branch (e.g., from a polluted branch to a clean one) but keep the same PR number, the head branch field is immutable after PR creation. `gh pr edit` supports `--body`, `--title`, `--base` (kinda), `--milestone`, `--add-reviewer`/etc., but NOT `--head`. `gh pr edit --head <new>` fails with "Unknown flag: --head".
